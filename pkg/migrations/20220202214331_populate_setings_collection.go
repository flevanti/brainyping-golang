...
func getListOfSettings() []SettingType {
	...
	settings := []SettingType{
		...
		{Key: "WRK_THROTTLE_LIMIT_RPS", Value: "50", Descr: "limit the number of request that each worker performs every second"},
		...
		{Key: "QUEUE_PREFETCH_SIZE", Value: "100", Descr: "number of messages to consume each request"},
		{Key: "BL_RPS_SPREAD", Value: "", Descr: "number of requests per seconds spread each second. this is used when bulk loading checks to avoid having all the checks/requests happening in the same second"},
		...
		{Key: "RC_BULK_SAVE_SIZE", Value: "100", Descr: "number of response records to include in each saving operation"},
